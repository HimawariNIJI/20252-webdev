...
@section('content')
    ...
    <form action="{{ route('insert_product') }}" class="row g-3"  method="POST" enctype="multipart/form-data">
        ...
        <div class="col-12">
            ...
            <input type="text" class="form-control" id="details" name="details" placeholder="Input product details here..." value="{{ old('details') }}">
        </div>
        <div class="col-md-6">
            <label for="product_category" class="form-label">Product Category</label>
            <select id="product_category" class="form-select @error('product_category') is-invalid @enderror" name="product_category" value="{{ old('product_category') }}">
              <option value="" selected disabled>Select a Category</option>
                @foreach ($product_categories as $pc)
                  <option value="{{ $pc->id }}" {{ old('product_category') == $pc->id ? 'selected' : '' }}>
                    {{ $pc->name }}
                  </option>
                @endforeach
            </select>
